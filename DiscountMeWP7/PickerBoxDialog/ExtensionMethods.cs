...
namespace Phone.Controls {
    public static class ExtensionMethods {
        ...
        public static IEnumerable<FrameworkElement> GetVisualAncestors(this FrameworkElement node) {
            FrameworkElement parent = GetVisualParent(node);
            while (parent != null) {
                yield return parent;
        ...
        public static VisualStateGroup GetAncestorOrSelfVisualStateGroup(this FrameworkElement root, string groupName) {
            ...
            foreach (FrameworkElement ancestor in GetVisualAncestors(root)) {
        ...
        public static bool TestItemVisibility(FrameworkElement item, FrameworkElement viewport, Orientation orientation,
                                              bool wantVisible) {
            ...
            Debug.WriteLine(String.Format("Check visibility {0}-{1} inside {2}-{3}, want visible {4} / result {5}", min,
                                          max, testMin, testMax, wantVisible, result));
        ...
        public static IEnumerable<T> GetVisibleItems<T>(this IEnumerable<T> items, FrameworkElement viewport,
                                                        Orientation orientation) where T : FrameworkElement {
            // Skip over the non-visible items, then take the visible items
            var skippedOverBeforeItems =
                items.SkipWhile((item) => TestItemVisibility(item, viewport, orientation, false));
            var keepOnlyVisibleItems =
                skippedOverBeforeItems.TakeWhile((item) => TestItemVisibility(item, viewport, orientation, true));
            return keepOnlyVisibleItems;
        }

        [Obsolete("Use Linq instead")]
        public static T GetAncestor<T>(this FrameworkElement root) where T : class {
            return GetVisualAncestors(root).FirstOrDefault(o => o is T) as T;
        }
        ...
        public static RequestedTransform GetTransform<RequestedTransform>(this UIElement element, bool create)
        ...
        public static RequestedTransform GetTransform<RequestedTransform>(this UIElement element,
